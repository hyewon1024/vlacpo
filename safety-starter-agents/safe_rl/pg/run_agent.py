...
from safe_rl.pg.failure_model import FailureCostModel
...
def run_polopt_agent(env_fn, 
    ...
    combined_input_dim = obs_dim + act_dim
    ...
    def feature_fn(o):
        return o
        # return np.concatenate([o, a], axis=-1).astype(np.float32)    

    failure_model = FailureCostModel(
        cfg_path="/home/hyewon/cpon_vla/scripts/batch_training/logs/safetygym_point-default-lstm-lstm/20260129/170428/config.yaml",
        ckpt_path="/home/hyewon/cpon_vla/scripts/batch_training/logs/safetygym_point-default-lstm-lstm/20260129/170428/model_final.ckpt",
        input_dim=88,
        feature_fn=feature_fn,
        window=20,
        device="cpu", 
    )
    print("failure_model can be loaded!")
    ...
    for epoch in range(epochs):
        ...
        for t in range(local_steps_per_epoch):
            ...
            # change to failure cost 
            c = failure_model(o)
            c = 0.1 * c  # scale 
            if t % 100 == 0 and proc_id() == 0:
                print(f"[Step {t:4d}] Action: {a}, Failure_Cost: {c:.4f}, Environment Info: {info}")
            
            # c = info.get('cost', 0) # 기존 코드 
        ...
        if proc_id() == 0:
            ...
            print(f"Average EpRet: {logger.get_stats('EpRet')[0]:.2f}")
            print(f"Average EpCost: {logger.get_stats('EpCost')[0]:.2f}")
            print(f"Policy KL: {logger.get_stats('KL')[0]:.6f}")
            if agent.use_penalty:
                print(f"Current Penalty: {cur_penalty:.4f}")
            print("-" * 30)
        #=====================================================================#
...
if __name__ == '__main__':
    ...
    parser.add_argument('--env', type=str, default='Safexp-CarPush2-v0')
    ...
    parser.add_argument('--cost_lim', type=float, default=60)
